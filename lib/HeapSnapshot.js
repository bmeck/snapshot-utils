...
export default class HeapSnapshot {
  ...
  constructor(provider) {
     ...
     * @private
     */
    this.Location = createLocationClass(this, provider);
    /**
     * @private
     */
  ...
  getNode(node_index) {
    ...
    return {
      ...
      getLocation: () => {
        const location_size = this.meta.location_fields.length;
        for (let i = 0; i < this.provider.getLocationArraySize() / location_size; i += location_size) {
          const buffer = this.provider.getLocationBuffer(i);
          const maybe = new this.Location(buffer);
          if (maybe.object_index === node_index) {
            return maybe;
          }
        }
        return null;
...
//   return TraceFrame;
// }
function createLocationClass(snapshot, provider) {
  const meta = provider.getMeta();
  const jsonStringify = eval(`(self, target) => {
    ${meta.location_fields.map(k => {
      const key = JSON.stringify(k);
      return `target[${key}] = self[${key}]`;
    }).join(';')}
    return target;
  ...
  class Location {
    ...
    [inspect.custom]() {
      let ret = '';
      let i = 0;
      for (const field of meta.location_fields) {
        ret +=
          ' ' +
          [
            field,
      ...
      return `HeapSnapshot::Location ${ret}`;
    }
  }
  let i = 0;
  for (let field of meta.location_fields) {
    (() => {
      const field_index = i * 4;
      i++;
      Object.defineProperty(Location.prototype, field, {
        enumerable: true,
        get() {
          try {
            return this._buffer.readUInt32BE(field_index);
