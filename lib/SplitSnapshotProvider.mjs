...
export default class SplitSnapshotProvider {
  ...
  constructor(
    ...
    if (sorted_ids) {
      ...
      for (let i = 0; i < snapshot.node_count; i++) {
        this.sorted_ids[i] = this.node_fields_length * i;
      }
      this.sorted_ids = this.sorted_ids.sort((offset_a, offset_b) => {
      ...
      for (let i = 0; i < snapshot.node_count; i++) {
        const offset = this.sorted_ids[snapshot.node_count + i];
  ...
  getNodeBuffer(index) {
    ...
    // console.log(n, index, this.node_fields_length);
    if (n !== (n | 0)) {
  ...
  getString(index) {
    ...
    // console.log('offset', index_offset);
  ...
  writeToDirectory(outdir, callback) {
    ...
    const sorted_ids_file = path.join(outdir, 'sorted_ids');
  ...
  static fromDirectory(outdir, callback) {
    ...
    fs.readFile(sorted_ids_file, (e, b) => {
      if (e) {
        done(e);
        return;
      }
