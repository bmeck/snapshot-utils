...
export default class HeapSnapshot {
  ...
  scan({ onNode = Function.prototype }) {
    const snapshot = this;
    return (function* scan() {
      for (let i = 0; i < snapshot.provider.node_arr_length; i++) {
        onNode(snapshot.getNode(i * snapshot.provider.node_fields_length));
        yield;
      }
    })();
  }
  ...
  getNodeById(node_id) {
    ...
    let node_index =
      Math.ceil(node_id / last.fields.id * node_arr_length) * node_fields_len;
    const incr =
