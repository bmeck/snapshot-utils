...
export default class SplitSnapshotProvider {
  ...
  constructor(
     ...
     * @private
     */
     this.location_fields_length = snapshot.meta.location_fields.length;
    // generally some locations will be missing, so this isn't verifiable
     /**
      * @private
      */
     this.location_arr_length = location_buffer.length;
     // already contains node buffer index
     /**
      * @private
      */
     this.location_struct_size = snapshot.meta.location_fields.length * 4;
     /**
      * @private
      */
     this.location_buffer = location_buffer;
  ...
  getLocationArraySize() {
    return this.location_arr_length;
  }
   ...
   * @return {Buffer}
   */
  getLocationBuffer(index) {
    let n = index / this.location_fields_length;
    if (n !== (n | 0)) {
      throw new RangeError('index is not on a Location boundary ' + n + ':' + index + ':' + this.location_fields_length);
    }
    let offset = n * this.location_struct_size;
    return this.location_buffer.slice(offset, offset + this.location_struct_size);
  }
  ...
  static fromDirectory(outdir) {
    ...
    const nodes_file = join(outdir, 'nodes');
    const edges_file = join(outdir, 'edges');
    const locations_file = join(outdir, 'locations');
    const samples_file = join(outdir, 'samples');
    const strings_file = join(outdir, 'strings');
    const string_indices_file = join(outdir, 'string_indices');
    const sorted_ids_file = join(outdir, 'sorted_ids');
  ...
  static fromStream(stream) {
    ...
    let locations_buffer_index = 0;
    let locations_buffer = null;
    ...
    return new Promise((fulfill, reject) => {
      stream
        .pipe(
          parseSnapshotStream({
            onsnapshot_info(snapshot) {
              ...
              nodes_buffer = Buffer.alloc(node_struct_size * snapshot.node_count);

              // prepend node_buffer index to get node
              const edge_struct_size = snapshot.meta.edge_fields.length * 4 + 4;
              edges_buffer = Buffer.alloc(edge_struct_size * snapshot.edge_count);
              
              // prepend node_buffer index to get node
              const location_struct_size = snapshot.meta.location_fields.length * 4;
              locations_buffer = Buffer.alloc(location_struct_size * snapshot.node_count);

              strings_indices_buffer = Buffer.alloc(4 * 1024);
              strings_buffer = Buffer.alloc(4 * 1024);
            },
            onnode(buffer) {
              if (buffer == null) {
                edge_offset = 0;
            ...
            onlocation(buffer) {
              ...
              locations_buffer.set(buffer, locations_buffer_index);
              locations_buffer_index += buffer.length;
            },
            onsample(buffer) {
              if (buffer == null) {
            ...
            onstring(str) {
              ...
              let str_buf = Buffer.from(str, 'utf8');
        ...
        .on('error', e => reject(e))
        .on('end', () => {
          const trimmed_locations_buffer = Buffer.alloc(locations_buffer_index);
          trimmed_locations_buffer.set(locations_buffer.slice(0, locations_buffer_index), 0);
          fulfill(
            new SplitSnapshotProvider(
              snapshot_info,
              nodes_buffer,
              edges_buffer,
              locations_buffer,//: trimmed_locations_buffer,
              samples_buffer,
              strings_indices_buffer,
              strings_buffer
            )
